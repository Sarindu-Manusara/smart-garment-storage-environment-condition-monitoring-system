...
#define MQ135_AO_PIN  32          // changed from 4 to 32
...
// Two 10kΩ resistors form a 1:1 divider, actual sensor voltage = ADC voltage * 2
const float MQ135_DIVIDER_RATIO = 2.0f;

// MQ-135 clean-air baseline (averaged raw ADC in stable clean-air conditions)
...
struct MQ135Data {
  int   raw;
  float adcVoltage;          // voltage seen at GPIO 32 (after divider)
  float sensorVoltage;       // actual voltage at MQ-135 AO (before divider)
  float airQualityDeviation; // % deviation from clean-air baseline (positive = worse air)
};

// Store last valid shown values
...
DustData readDustAveraged(int samples) {
  long sum = 0;
  for (int i = 0; i < samples; i++) sum += readDustRawSingle();
